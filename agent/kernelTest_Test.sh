...
PROG_PATH="${PROG_PATH:-$SCRIPT_DIR/bin/kernelTest}"
REPORT_DIR="${REPORT_DIR:-$SCRIPT_DIR/ncu}"
REPORT_BASENAME="${REPORT_BASENAME:-kernelTest}"
CSV_PAGE="${CSV_PAGE:-raw}"
CUDA_DEVICE="${CUDA_VISIBLE_DEVICES:-0}"
USE_SUDO="${USE_SUDO:-1}"

DEFAULT_CASE="4096 4096 1024 1"

usage() {
    cat <<'EOF'
Usage:
  ./kernelTest_Test.sh
  ./kernelTest_Test.sh M K N SPLIT_K

Examples:
  ./kernelTest_Test.sh
  ./kernelTest_Test.sh 4096 4096 1024 1

Environment variables:
  CUDA_VISIBLE_DEVICES=0   GPU device selection (default: 0)
  NCU_PATH=...             Path to Nsight Compute CLI
  PROG_PATH=...            Executable to profile (default: ./bin/kernelTest)
  REPORT_DIR=...           Report output directory (default: ./ncu)
  REPORT_BASENAME=...      Report filename prefix (default: kernelTest)
